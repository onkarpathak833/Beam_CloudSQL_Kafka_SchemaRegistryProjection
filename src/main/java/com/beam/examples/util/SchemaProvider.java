...
//        String kafkaSchemaRegistry = properties.getProperty(Constants.SCHEMA_REGISTRY_URL);
        ...
        try {
            ...
            for (Schema.Field field : schemaFields) {
                ...
                Schema.Field typeField = null;
                if (checkIfPrimitive(field.schema().getType())) {
                    if (field.schema().getType().equals(Schema.Type.INT) || field.schema().getType().equals(Schema.Type.LONG)) {
                        typeField = new Schema.Field(field.name(), field.schema(), null, 0);
                    }
                    else if(field.schema().getType().equals(Schema.Type.DOUBLE)) {
                        typeField = new Schema.Field(field.name(), field.schema(), null, 0.0);
                    }
                    else if(field.schema().getType().equals(Schema.Type.BOOLEAN)) {
                        typeField = new Schema.Field(field.name(), field.schema(), null, false);
                    }
                    else {
                        typeField = new Schema.Field(field.name(), field.schema(), null, Schema.Type.NULL);
                    }

                    generatedSchemaFields.add(typeField);
                } else {
                    typeField = new Schema.Field(field.name(), Schema.create(Schema.Type.INT), null, 0);
                    generatedSchemaFields.add(typeField);
                }
    ...
    public List<GenericRecord> createGenericRecord(String tableSchema, ResultSet resultSet, Map<String, String> properties) {
